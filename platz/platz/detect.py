"""Classes that implement different place field detection strategies"""

# At the moment this is just copied verbatim from lab3...
...
from .util import find_intervals
from .tuning import SpatialTuningStrategy, CircularMixin
# from .metric import spatial_information, spatial_reliability, first_lap
...
class PlaceFieldStrategy(metaclass=PlaceFieldMeta):  # set base classes
...
class SimplePFDetector(PlaceFieldStrategy):
    ...
    def _find_fields(self, tuning_curves=None, threshold_curves=None):
        ...
        self.place_fields = pd.DataFrame(
            place_fields, columns=['roi_label', 'field_count',
                                   'start_bin', 'stop_bin']).set_index(
            ['roi_label', 'field_count']).sort_index()
