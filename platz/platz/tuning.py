"""Classes for computing spatial tuning profiles from neural data"""
...
# from astropy.convolution import convolve  # to handle nans


class SpatialTuningStrategy(metaclass=ABCMeta):
...
class SimpleSpatialTuning(SpatialTuningStrategy):
    ...
    def _shuffle(self, signals=None, position=None, laps=None):
        ...
        shuffled_position = np.concatenate(
                [np.roll(position[(laps == l)],
                         np.random.randint((laps == l).sum()))
    ...
    def calculate_rasters(self, signals, position, laps, smooth=True,
                          fill_nans=True, max_position=None):
        """doc string
        ...
        # insert NaNs for missing position bins in the tuning curve
        if max_position is None:
            max_position = np.max(position).astype('int')

        missing_bins = []
        for n in range(max_position):
            if n not in rasters.columns:
                missing_bins.append(n)
                rasters.loc[:, n] = np.nan
        rasters = rasters.sort_index(axis=1)
