...
def frame_writer(out_file, first_obs, seq_dictionary):
    ...
    f = np.memmap(out_file, mode='r+', dtype=np.float32, order='C', # or C? who knows...
                  shape=(shape[0], num_f))
...
def convert(in_file, out_file, cycle=0, chunk_size=500, n_processes=4, channel='Ch2',
            first_obs=150, trim=0, z_plane=None, skip_init=0, n_frames=None):
            # time_avg=False?
    ...
    with multiprocessing.get_context("spawn").Pool(processes=n_processes) as pool:
        ...
        pool.map(
            pool_helper,
            zip(it.repeat(frame_writer), it.repeat(out_file), it.repeat(first_obs),
                sequences))
